...
#define ERR_VERIFY(cond) \
    do { \
        if (!(cond)) { \
            callErrorListeners(__FUNCTION__, __FILE__, __LINE__, "Condition \"" #cond "\" is false."); \
            APP_DEBUGBREAK(); \
            return; \
        } \
    } while (0)
...
#define ERR_VERIFY_V(cond, returnValue) \
    do { \
        if (!(cond)) { \
            callErrorListeners(__FUNCTION__, __FILE__, __LINE__, "Condition \"" #cond "\" is false, returning \"" #returnValue "\"."); \
            APP_DEBUGBREAK(); \
            return (returnValue); \
        } \
    } while (0)
...
#define ERR_VERIFY_NULL(m_pointer) \
    do { \
        if (!(m_pointer)) { \
            callErrorListeners(__FUNCTION__, __FILE__, __LINE__, "\"" #m_pointer "\" is null."); \
            APP_DEBUGBREAK(); \
            return; \
        } \
    } while (0)
...
#define ERR_VERIFY_NULL_V(m_pointer, m_returnValue) \
    do { \
        if (!(m_pointer)) { \
            callErrorListeners(__FUNCTION__, __FILE__, __LINE__, "\"" #m_pointer "\" is null, returning \"" #m_returnValue "\""); \
            APP_DEBUGBREAK(); \
